...
<head>
    ...
    <title>Zarządzanie Użytkownikami - MeScrap</title>
...
<body>
    ...
    <div class="page-header">
        <div class="container">
            <h1 class="page-title display-4">Zarządzanie Użytkownikami</h1>
            <p class="lead" style="color: var(--text-main);">Przeglądaj i zarządzaj kontami użytkowników</p>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            ...
            <a href="{{ route('admin.users.inactive') }}" class="btn btn-warning">Nieaktywni (6+ mies.)</a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">Lista wszystkich użytkowników ({{ $users->total() }})</h5>
        </div>
        <div class="card-body">
                    ...
                    <div class="col-md-8">
                        <div class="input-group">
                            <input type="text" 
                                   name="search" 
                                   class="form-control" 
                                   placeholder="Szukaj po nazwie, emailu lub domenie (np. gmail.com)..."
                                   value="{{ request('search') }}"
                                   aria-label="Pole wyszukiwania użytkowników">
                ...
                @if(request('search'))
                    <div class="mt-2">
                        <small class="text-muted">
                            Znaleziono {{ $users->total() }} użytkowników dla frazy: <strong>"{{ request('search') }}"</strong>
                        </small>
                    </div>
                @endif
            </form>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            <th style="cursor: pointer;">
                                <a href="{{ route('admin.users.index', array_merge(request()->query(), ['sort' => 'id', 'dir' => $sortBy === 'id' && $sortDir === 'asc' ? 'desc' : 'asc'])) }}" style="color: inherit; text-decoration: none;">
                                    ID
                                    @if($sortBy === 'id')
                            ...
                            <th style="cursor: pointer;">
                                <a href="{{ route('admin.users.index', array_merge(request()->query(), ['sort' => 'name', 'dir' => $sortBy === 'name' && $sortDir === 'asc' ? 'desc' : 'asc'])) }}" style="color: inherit; text-decoration: none;">
                                    Nazwa użytkownika
                                    @if($sortBy === 'name')
                                        <i class="bi {{ $sortDir === 'asc' ? 'bi-arrow-up' : 'bi-arrow-down' }}"></i>
                                    @endif
                            ...
                            <th style="cursor: pointer;">
                                <a href="{{ route('admin.users.index', array_merge(request()->query(), ['sort' => 'email', 'dir' => $sortBy === 'email' && $sortDir === 'asc' ? 'desc' : 'asc'])) }}" style="color: inherit; text-decoration: none;">
                                    Email
                                    @if($sortBy === 'email')
                                        <i class="bi {{ $sortDir === 'asc' ? 'bi-arrow-up' : 'bi-arrow-down' }}"></i>
                                    @endif
                            ...
                            <th>Rola</th>
                            <th style="cursor: pointer;">
                                <a href="{{ route('admin.users.index', array_merge(request()->query(), ['sort' => 'created_at', 'dir' => $sortBy === 'created_at' && $sortDir === 'asc' ? 'desc' : 'asc'])) }}" style="color: inherit; text-decoration: none;">
                                    Data rejestracji
                            ...
                            <th style="cursor: pointer;">
                                <a href="{{ route('admin.users.index', array_merge(request()->query(), ['sort' => 'last_login_at', 'dir' => $sortBy === 'last_login_at' && $sortDir === 'asc' ? 'desc' : 'asc'])) }}" style="color: inherit; text-decoration: none;">
                                    Ostatnie logowanie
                    ...
                    <tbody>
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="text-warning">Nigdy</span>
                                    @endif
                                </td>
                                <td>
                                    @php
                                        $sixMonthsAgo = \Carbon\Carbon::now()->subMonths(6);
                                        $isInactive = !$user->last_login_at || $user->last_login_at < $sixMonthsAgo;
                                    @endphp
                                    @if($isInactive)
                                        <span class="badge bg-warning text-dark">Nieaktywny</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center">Brak użytkowników do wyświetlenia.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
